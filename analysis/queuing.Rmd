...
A consumable resource is a resource that Grid Engine tracks and subtracts as jobs are scheduled. For example if the host has 128GB memory:

* Job 1 requests 64GB so Grid Engine marks 64GB as used
* Job 2 requests 64GB so Grid Engine marks another 64GB as used
* Job 3 requests 64GB and Grid Engine blocks it (not enough left)

To check whether a resource is consumable, run:
...
If a resource under consumable is `YES` then it is a consumable and Grid Engine tracks it. On my default installation, only `slots` is consumable.
